...
//  result = vmull_p64(aa, bb);
	...
	imm = imm & 0x11;
	const __m128i testresult = (imm == 0x10) ? _mm_clmulepi64_si128_emu(tmp1, tmp2, 0x10) : ((imm == 0x01) ? _mm_clmulepi64_si128_emu(tmp1, tmp2, 0x01) : ((imm == 0x00) ? _mm_clmulepi64_si128_emu(tmp1, tmp2, 0x00) : _mm_clmulepi64_si128_emu(tmp1, tmp2, 0x11)));
	...
	uint32_t *fixrand, uint32_t *fixrandex, __m128i *g_prand, __m128i *g_prandex)
{
	...
	__m128i acc = _mm_load_si128_emu(randomsource + 513);
	...
	for (uint32_t i = 0; i < 32; i++)
	{
		...
		case 0:
		{
			...
			const   __m128i temp12 = _mm_load_si128_emu(prand);
			*prand = _mm_xor_si128_emu(tempa1, temp1);
			//   _mm_store_si128_emu(prand, tempa2);
			...
			*prandex = _mm_xor_si128_emu(tempb1, temp12);
			//  _emu_mm_store_si128_emu(prandex, tempb2);
		...
		case 0xc:
		{
			...
			else
			{
				...
				*prand = _mm_load_si128_emu(prandex);
				_mm_store_si128_emu(prandex, tempa2);
				//  _mm_store_si128_emu(prand, tempb3);
			}
		...
		case 0x10:
		{
			...
			acc = _mm_xor_si128_emu(temp2, _mm_xor_si128_emu(temp1, acc));
		...
		case 0x14:
		{
			...
			do
			{
				...
				else
				{
					...
					AES2_EMU(onekey, temp2, aesroundoffset);
		...
		case 0x18:
		{
			...
			do
			{
				...
				{
					onekey = _mm_load_si128_emu(rc++);
					__m128i temp2 = _mm_load_si128_emu(rounds & 1 ? pbuf : buftmp);
					__m128i add1 = _mm_xor_si128_emu(onekey, temp2);
					// cannot be zero here, may be negative
					int32_t divisor = (uint32_t)selector;
					int64_t dividend = _mm_cvtsi128_si64_emu(add1);
					__m128i modulo = _mm_cvtsi32_si128_emu(dividend % divisor);
					acc = _mm_xor_si128_emu(modulo, acc);
				}
				else
				{
					...
					__m128i clprod2 = _mm_mulhrs_epi16_emu(acc, clprod1);
					acc = _mm_xor_si128_emu(clprod2, acc);
				}
			} while (rounds--);

			__m128i tempa3 = _mm_load_si128_emu(prandex);
			*prandex = _mm_xor_si128_emu(tempa3, acc);
	...
	uint32_t *fixrand, uint32_t *fixrandex, __m128i *g_prand, __m128i *g_prandex)
{
	...
	for (int64_t i = 0; i < 32; i++)
	{
		...
		case 4:
		{
			...
			const __m128i tempb1 = _mm_mulhrs_epi16_emu(acc, temp12);
			_mm_store_si128_emu(prand,_mm_xor_si128_emu(tempb1, temp12));
			//_mm_store_si128_emu(prand, tempb2);
			break;
		}
		case 8:
		...
		case 0xc:
		{
			...
			{
				_mm_store_si128_emu(prand, _mm_load_si128_emu(prandex));
				_mm_store_si128_emu(prandex, tempa2);
				acc = _mm_xor_si128_emu(_mm_load_si128_emu(pbuf), acc);
			}
			break;
		}
		case 0x10:
		{
			...
			_mm_store_si128_emu(prand, _mm_load_si128_emu(prandex));
			_mm_store_si128_emu(prandex, _mm_xor_si128_emu(tempa1, tempa2));
		...
		case 0x14:
		{
			...
			do
			{
				if (selector & (((uint64_t)0x10000000) << rounds))
				{
					//onekey = _mm_load_si128_emu(rc++);
					const __m128i temp2 = _mm_load_si128_emu(rounds & 1 ? pbuf : buftmp);
					const __m128i add1 = _mm_xor_si128_emu(rc[0], temp2); rc++;
					const __m128i clprod1 = _mm_clmulepi64_si128_emu(add1, add1, 0x10);
					acc = _mm_xor_si128_emu(clprod1, acc);
				}
				else
		...
		case 0x18:
		{
			...
			do
			{
				if (selector & (((uint64_t)0x10000000) << rounds))
				{
					//	onekey = _mm_load_si128_emu(rc++);
					const __m128i temp2 = _mm_load_si128_emu(rounds & 1 ? pbuf : buftmp);
					onekey = _mm_xor_si128_emu(rc[0], temp2); rc++;
					// cannot be zero here, may be negative
					const int32_t divisor = (uint32_t)selector;
					const int64_t dividend = _mm_cvtsi128_si64_emu(onekey);
					const __m128i modulo = _mm_cvtsi32_si128_emu(dividend % divisor);
			...
			_mm_store_si128_emu(prandex, onekey);
			_mm_store_si128_emu(prand, _mm_xor_si128_emu(tempa3, acc));
			//	_mm_store_si128_emu(prand, tempa4);
			break;
		}
		case 0x1c:
		{
			...
			acc = _mm_xor_si128_emu(temp4, acc);
			const __m128i tempb1 = _mm_mulhrs_epi16_emu(acc, tempa3);
			*prandex = _mm_xor_si128_emu(tempb1, tempa3);
			//	_mm_store_si128_emu(prandex, tempb2);
			break;
		}
		}
...
uint64_t verusclhashv2_1(void * random, const unsigned char buf[64], uint64_t keyMask, uint32_t *fixrand, uint32_t *fixrandex,
	...
	acc = _mm_xor_si128_emu(acc, lazyLengthHash(1024, 64));
